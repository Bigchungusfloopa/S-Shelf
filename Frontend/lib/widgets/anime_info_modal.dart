...
import 'anime_dialog.dart';

class AnimeInfoModal extends StatelessWidget {
  final Anime anime;
  final VoidCallback onUpdate;

  const AnimeInfoModal({
    Key? key,
    required this.anime,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _getStatusColor(anime.status).withOpacity(0.2),
                        borderRadius: BorderRadius.circular(20),
                        border: Border.all(color: _getStatusColor(anime.status)),
                      ),
                      child: Text(
                        _getStatusLabel(anime.status),
                        style: TextStyle(
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: _buildInfoItem(
                            'Your Score',
                            anime.userScore?.toStringAsFixed(1) ?? 'N/A',
                            Icons.star_outlined,
                            themeProvider,
                          ),
                        ),
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () async {
                        ...
                        if (confirm == true) {
                          final success = await apiService.deleteAnime(anime.id);
                          Navigator.pop(context);
                          if (success) {
                            onUpdate();
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('Anime deleted')),
                            );
                          }
                        }
                      },
